...
Stepper_WakeUp(void) {
	...
	if(BIT_IS_TRUE(settings.flags, BITFLAG_INVERT_ST_ENABLE)) {
        ...
        HAL_GPIO_WritePin(GPIO_ENABLE_PORT, GPIO_ENABLE_PIN, GPIO_PIN_RESET);
	}
...
Stepper_MainISR(void) {
    ...
    if(st.step_outbits & (1<<Y_STEP_BIT)) {
		if(step_port_invert_mask & (1<<Y_STEP_BIT)) {
			...
			// High pulse
            HAL_GPIO_WritePin(GPIO_STEP_Y_PORT, GPIO_STEP_Y_PIN, GPIO_PIN_SET);
		}
    ...
    if(st.step_outbits & (1<<Z_STEP_BIT)) {
		if(step_port_invert_mask & (1<<Z_STEP_BIT)) {
			...
			// High pulse
            HAL_GPIO_WritePin(GPIO_STEP_Z_PORT, GPIO_STEP_Z_PIN, GPIO_PIN_SET);
		}
    }
